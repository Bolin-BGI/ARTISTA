...
setwd('../data/Figure4_data/Figure4E_Monocle3')
...
### NPTXEX
root = 'NPTXEX'
data = subset(data,idents= c('REAEGC','RIPC1','IMN','NPTXEX'))
...
obj <- RunPCA(obj, verbose = FALSE,npcs = 6)
obj <- RunUMAP(obj, dims = 1:6, verbose = FALSE,min.dist = 0.3)
...
### color
co = read.csv('../data/F_Monocle3/Inj_24_Adult_Develop_color_0305.txt',sep='\t')
...
get_earliest_principal_node <- function(cds, time_bin=c('REAEGC')){
